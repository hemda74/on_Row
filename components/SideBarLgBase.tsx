...
const SideBarLg: React.FC<WorkspaceArray> = ({ workspaceData, id }) => {
  ...
  const handledescChange = (event: ChangeEvent<HTMLTextAreaElement>) => {
    ...
    const descErrorMessageElement = document.getElementById("descErrorMessage");
  ...
  const handleSubmit = async () => {
    if (name.length < 1) {
      ...
      try {
        ...
        toast.error("Failed to create base. Please try again later.");
      }
    }
  };
      ...
      <div className="flex h-full max-h-screen flex-col gap-2">
        <div className="flex h-14 items-center border-b px-4 lg:h-[60px] lg:px-6">
          <Link href="/" className="flex items-center gap-2 font-semibold">
            ...
            <span className="text-primary">onRow</span>
          </Link>
        </div>
        <div className="flex-1 bg-white">
          <nav className="grid items-start px-2 text-sm font-medium lg:px-4">
            ...
            <Link
              ...
              className="flex items-center gap-3 rounded-lg px-3 py-2 text-muted-foreground transition-all hover:text-primary"
            >
              <LineChart className="h-4 w-4" /> Bases
        ...
        <div className="mt-auto p-4">
          <CardHeader className="p-2 pt-0 md:p-4">
            ...
            <CardDescription>
              Add New Base and get unlimited access to our support team.
            </CardDescription>
          </CardHeader>
          <CardContent className="p-2 pt-0 md:p-4 md:pt-0">
            <AlertDialog>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  <AlertDialogTitle>Add Base</AlertDialogTitle>
                  <AlertDialogDescription>
                    <CardContent className="grid gap-4">
                      ...
                      <div className="grid gap-2">
                        <Label htmlFor="desc">Base Description</Label>
                        <Textarea
                          id="desc"
